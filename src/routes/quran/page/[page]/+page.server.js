...
export const load = async ({ params, fetch }) => {
	...
	const fetchSurah = async (num) => {
		const req = await fetch(`${url}/api/v1/quran/ara-qurandoorinonun/page/${num}?audio=Alafasy_128kbps&tafsir=en-tafisr-ibn-kathir`)
		const res = await req.json()
		const transReq = await fetch(`${url}/api/v1/quran/eng-abdullahyusufal/page/${num}?audio=Alafasy_128kbps&tafsir=en-tafisr-ibn-kathir`)
