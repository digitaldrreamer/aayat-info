...
<div class="min-h-screen bg-background text-foreground font-primary">
    ...
    <!-- Main Content Area -->
    <main class="pt-14 pb-16 md:pb-20 container max-w-screen-2xl mx-auto p-4">
        <!-- Content goes here -->
    </main>

    <!-- Mobile Bottom Navigation -->
    <nav
            class="fixed md:hidden bottom-4 left-4 right-4 h-14 bg-background border rounded-full shadow-lg z-50"
            in:fly={{ y: 20, duration: 300, easing: quintOut }}
    >
        <div class="relative h-full flex items-center justify-between px-6">
            ...
            <div class="absolute left-1/2 -translate-x-1/2 -top-4">
                <Button
                        class="h-16 w-16 rounded-full bg-neutral-900 hover:bg-neutral-800 text-neutral-50"
                        onclick={() => open = true}
                >
                    <Plus class="w-6 h-6" />
                </Button>
    ...
    <Command.Dialog bind:open>
        <Command.Input placeholder="Search Quran..." />
        <Command.List>
